...
import { createBook, getBooks, newWebSocket, updateBook, syncData } from './BookApi';
import { Storage } from '@capacitor/storage';
import { Network } from '@capacitor/network';

const log = getLogger('BookProvider');
type SaveBookFct = (book: any) => Promise<any>;
...
export const BookProvider: React.FC<bookProviderProps> = ({ children }) => {
  ...
  const saveBook = useCallback<SaveBookFct>(saveBookCallback, [token]);
  ...
  async function saveBookCallback(book: BookProps) {
    try {
      if (navigator.onLine) {
          ...
          window.alert('Saving the book failed because you are offline.\n' +
                    'Book is saved to the local storage until successful connection');
  ...
  function webSocketsEffect() {
    let canceled = false;
    log('webSocketsEffect - connecting');
    ...
    if (token?.trim()) {
      closeWebSocket = newWebSocket(token, message => {
        ...
        log(`webSocketsEffect message, book ${type}`);
    ...
    return () => {
      log('webSocketsEffect - disconnecting');
      canceled = true;
      closeWebSocket?.();
    }
  }
